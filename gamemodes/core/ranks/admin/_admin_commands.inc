...
Command(name: setrank, rank: Rank_Administrator(), args: playerid, const string: params[])
{
	...
	if (sscanf(params, "rs[24]", targetid, rank))
		return SendSyntaxMsg(playerid, "/setrank [targetid] [rank (igrac|vip|moderator|admin)]");
	...
	else if (!strcmp(rank, "vip", false))
		Rank_Set(targetid, playerid, "vip");
...
Command(name: ah, rank: Rank_Administrator(), args: playerid, const string: params[])
{
	Dialog_Show(playerid, "DIALOG_ADMINHELP", DIALOG_STYLE_MSGBOX,
		...
		"YELLOW"Administrator: "WHITE"/kick /vrespawn /jetpack /pm /setrank /playmusic /a /givegun /kreirajport\n\
		"YELLOW"Administrator: "WHITE"/kreirajkiosk /obrisikiosk /izmenikiosk /portkiosk /goto /gethere /setstats\n\
		"YELLOW"Administrator: "WHITE"/setorg /setorgrank /kreirajgps /hungerall /thirstall /setskin /port /dmevent /stopdmevent\n\
		%s\n\
		"YELLOW"______________________________________________________",
		""YELLOW"Zatvori", "",
...
Command(name: kreirajport, rank: Rank_Administrator(), args: playerid, const string: params[])
{
	...
	GetPlayerPos(playerid, x, y, z);
	Port_Create(portid, params, x, y, z);
...
Command(name: testwl, rank: Rank_Administrator(), args: playerid, const string: params[])
{
	SendInfoMsgF(playerid, "Wanted: %d", WantedLevel_Get(playerid));
	return 1;
}